...
var DashboardPopups = function() {
    this.windowStorage = new WindowStorage('relatedWindows');
};

DashboardPopups.prototype = {
    ...
    closePopup: function(response) {
        console.log('CLOSE DASHBOARD POPUPJES', response);
...
//                 }
    ...
    initLookupLinks: function() {
        ...
        $("a[data-dashboard-popup-opener]").click(function(e) {
            e.preventDefault();
            self.showPopup('<input id="popup-input" type="hidden"/>', $(this).attr('href') + '?_popup=1');
        });
    },
    run: function() {
